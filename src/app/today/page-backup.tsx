...
function TaskSection({ title, tasks, emptyMessage, urgent = false, unifiedTasks, handleEditTask }: TaskSectionProps) {
      ...
      {tasks.length === 0 ? (
        ...
        <div style={{
          ...
          {tasks.map((task, index) => {
            const taskType = task.task_type === 'IDEA' ? 'アイデア' :
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: urgent || isOverdue ? 'rgba(239, 68, 68, 0.1)' :
                             isToday ? 'rgba(245, 158, 11, 0.1)' : 'transparent',
              ...
              >
                ...
                <div>
                  <input
                    ...
                    onChange={() => {
                      if (task.completed) {
                        unifiedTasks.uncompleteTask(task.id)
                      } else {
                        unifiedTasks.completeTask(task.id)
                      }
                    }}
                    style={{
                      width: '16px',
                ...
                <div>
                  <span style={{
                    background: task.task_type === 'IDEA' ? '#8b5cf6' :
                               task.task_type === 'RECURRING' ? '#10b981' : '#3b82f6',
                    color: 'white',
                    padding: '2px 6px',
                    borderRadius: '12px',
                    fontSize: '10px',
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`「${task.title}」を削除しますか？`)) {
                        unifiedTasks.deleteTask(task.id)
                      }
                    }}
                    style={{
                      padding: '4px 6px',
...
export default function TodayPage() {
  ...
  // ShoppingTasksSectionからのサブタスク更新通知を処理
  const handleShoppingSubTaskUpdate = useCallback(async (taskId: string) => {
